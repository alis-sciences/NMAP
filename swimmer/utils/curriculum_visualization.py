...
def create_trajectory_analysis(agent, env, save_path, num_steps=500, phase_name="", trajectory_multiplier=1.0):
    ...
    # ── Mean water bout length ────────────────────────────────────────────────
    # Distinguishes genuine swimming (long bouts) from boundary-touching
    # behaviour (many short bouts).  >50 steps = genuine swim; <10 = retreat.
    _water_bouts = []
    _in_water_bout = False
    _bout_len = 0
    for _et in env_types:
        if _et == 'Water':
            ...
            if _in_water_bout and _bout_len > 0:
                _water_bouts.append(_bout_len)
            _in_water_bout = False
            _bout_len = 0
    if _in_water_bout and _bout_len > 0:
        _water_bouts.append(_bout_len)
    mean_water_bout = float(np.mean(_water_bouts)) if _water_bouts else 0.0
    num_water_bouts = len(_water_bouts)
    ax4.text(0.02, 0.85, f'Mean water bout: {mean_water_bout:.1f} steps',
    ...
    return {
        ...
        'mean_water_bout': mean_water_bout,
        'num_water_bouts': num_water_bouts,
    }
    ...
    with open(save_path, 'w') as f:
        ...
        if 'trajectory_stats' in training_history:
            ...
            for phase, stats in training_history['trajectory_stats'].items():
                ...
                f.write(f"  - Mean Water Bout: {stats.get('mean_water_bout', 0.0):.1f} steps\n")
                f.write(f"  - Num Water Bouts: {stats.get('num_water_bouts', 0)}\n\n")
...
def create_body_profile_figure(agent, env, save_path,
                               num_steps=600, n_snapshots=20,
                               phase_name=''):
    """
    Collect a rollout and render 4-row 2-D body-profile figure.

    Row 1 — n_snapshots stick-figure dorsal-view panels, one per sampled
             timestep.  Background tint: blue = water, orange = land.
             Head = filled circle; joints connected by line.
    Row 2 — Continuous 2-D head trajectory coloured by medium.
             Land zones drawn as filled circles.
    Row 3 — Joint-angle kymograph (joint index × time, RdBu_r).
             Vertical dashed lines mark the Row-1 snapshot times.
    Row 4 — Instantaneous speed with medium background shading.
    """
    ...
    # detect n_joints from env action space
    try:
    ...
    body_positions_list = []
    velocities_list = []
    env_labels_list = []   # 0=water, 1=land
    land_zones_seen = []

    seg_names_base = ['head'] + [str(i) for i in range(1, n_joints + 2)]

    for _ in range(num_steps):
        try:
            # navigate env wrappers: TonicWrapper -> ProgressiveMixedSwimmerEnv -> dm_control env
            inner = env.env if hasattr(env, 'env') else env
            dm_env = inner.env if hasattr(inner, 'env') else inner
            physics = dm_env.physics
            task    = getattr(dm_env, '_task', None)

            hp = physics.named.data.xpos['head'][:2].copy()
            head_positions_list.append(hp)

            hv = physics.named.data.sensordata['head_vel'][:2]
            velocities_list.append(float(np.linalg.norm(hv)))

            qpos = physics.data.qpos[3:3 + n_joints]
            joint_angles_list.append(qpos.copy())

            # body segment world positions
            segs = []
            for sn in seg_names_base[:n_joints + 1]:
                try:
                    segs.append(physics.named.data.xpos[sn][:2].copy())
                except KeyError:
                    segs.append(hp.copy())
            body_positions_list.append(np.array(segs))

            # environment label
            lz = []
            if task is not None and hasattr(task, '_current_land_zones'):
                lz = task._current_land_zones or []
            if lz and not land_zones_seen:
                land_zones_seen = lz
            in_land = any(
                np.linalg.norm(hp - np.array(z['center'])) < z['radius']
                for z in lz
            )
            env_labels_list.append(1 if in_land else 0)

        except Exception:
            head_positions_list.append(np.zeros(2))
            velocities_list.append(0.0)
            joint_angles_list.append(np.zeros(n_joints))
            body_positions_list.append(np.zeros((n_joints + 1, 2)))
            env_labels_list.append(0)
    ...
    T = len(head_positions_list)
    if T < 4:
        print('⚠️  Body profile: insufficient data, skipping.')
        return

    head_pos  = np.array(head_positions_list)
    joint_ang = np.array(joint_angles_list)
    body_pos  = np.array(body_positions_list)
    vels      = np.array(velocities_list)
    env_lbl   = np.array(env_labels_list)

    snap_idx = np.linspace(0, T - 1, n_snapshots, dtype=int)

    # ── figure layout ─────────────────────────────────────────────────────────
    n_cols_snap  = min(n_snapshots, 10)
    n_rows_snap  = int(np.ceil(n_snapshots / n_cols_snap))
    panel_h      = n_rows_snap * 2.2

    fig = plt.figure(figsize=(max(16, n_cols_snap * 1.7),
                               panel_h + 9.5))
    outer = gridspec.GridSpec(4, 1,
                              height_ratios=[panel_h, 3.5, 2.8, 1.4],
                              hspace=0.50, left=0.06, right=0.97,
                              top=0.93, bottom=0.04)

    # ── ROW 1: stick figures ──────────────────────────────────────────────────
    snap_gs = gridspec.GridSpecFromSubplotSpec(
        n_rows_snap, n_cols_snap,
        subplot_spec=outer[0], hspace=0.08, wspace=0.05)

    # axis limits: max body span across all snapshots (relative to head)
    all_rel = body_pos[snap_idx] - body_pos[snap_idx, 0:1, :]
    max_span = max(0.25, float(np.abs(all_rel).max()) * 1.25)

    for k, t_i in enumerate(snap_idx):
        r, c = divmod(k, n_cols_snap)
        ax = fig.add_subplot(snap_gs[r, c])

        is_land = bool(env_lbl[t_i])
        bg      = CRAWL_C if is_land else SWIM_C
        ax.set_facecolor(bg + '20')

        rel = body_pos[t_i] - body_pos[t_i][0]   # centre on head
        xs, ys = rel[:, 0], rel[:, 1]

        ax.plot(xs, ys, '-', color='#333', linewidth=2.2, zorder=2)
        ax.plot(xs[1:], ys[1:], 'o', color='#666', markersize=3.5, zorder=3)
        ax.plot(0, 0, 'o', color=bg, markersize=8,
        ...
        ax.set_aspect('equal')
        ax.axis('off')
        lbl = 'crawl' if is_land else 'swim'
        ax.set_title(f't={t_i}\n{lbl}', fontsize=6.5, color=bg, pad=1)

    # ── ROW 2: head trajectory ────────────────────────────────────────────────
    ax_tr = fig.add_subplot(outer[1])
    ax_tr.set_title('Head trajectory  (blue = water, orange = land)',
                    fontsize=10)

    for z in land_zones_seen:
        circ = plt.Circle(z['center'], z['radius'],
                          fc=CRAWL_C + '28', ec=CRAWL_C, lw=1.4, zorder=1)
        ax_tr.add_patch(circ)

    for i in range(T - 1):
        c = CRAWL_C if env_lbl[i] else SWIM_C
        ax_tr.plot(head_pos[i:i+2, 0], head_pos[i:i+2, 1],
                   '-', color=c, lw=1.2, alpha=0.75)

    for t_i in snap_idx:
        c = CRAWL_C if env_lbl[t_i] else SWIM_C
        ax_tr.plot(head_pos[t_i, 0], head_pos[t_i, 1],
                   'o', color=c, ms=5, mec='white', mew=0.8, zorder=5)

    ax_tr.plot(*head_pos[0],  'k^', ms=9, label='start', zorder=6)
    ax_tr.plot(*head_pos[-1], 'ks', ms=9, label='end',   zorder=6)
    ax_tr.set_aspect('equal')
    ax_tr.grid(True, alpha=0.22)
    ax_tr.legend(fontsize=8, loc='upper right')
    ax_tr.set_xlabel('X (m)')
    ax_tr.set_ylabel('Y (m)')

    # ── ROW 3: joint-angle kymograph ─────────────────────────────────────────
    ax_ky = fig.add_subplot(outer[2])
    ax_ky.set_title('Joint-angle kymograph  (head → tail)', fontsize=10)

    ja = joint_ang.copy()
    for j in range(n_joints):
        col = ja[:, j]
        rng = col.max() - col.min() + 1e-8
        ja[:, j] = (col - col.min()) / rng * 2 - 1   # normalise → [-1,1]

    im = ax_ky.imshow(ja.T, aspect='auto', origin='lower',
                      cmap='RdBu_r', vmin=-1, vmax=1,
                      extent=[0, T, 0, n_joints])
    plt.colorbar(im, ax=ax_ky, fraction=0.018, pad=0.01,
                 label='norm. angle')

    for t_i in snap_idx:
        ax_ky.axvline(t_i, color='white', lw=0.6, ls='--', alpha=0.55)

    ax_ky.set_xlabel('time (steps)')
    ax_ky.set_ylabel('joint')
    ax_ky.set_yticks(np.arange(n_joints) + 0.5)
    ax_ky.set_yticklabels([f'J{i+1}' for i in range(n_joints)], fontsize=8)

    # ── ROW 4: speed + medium bar ─────────────────────────────────────────────
    ax_v = fig.add_subplot(outer[3])
    t_ax = np.arange(T)

    for i in range(T - 1):
        c = CRAWL_C if env_lbl[i] else SWIM_C
        ax_v.axvspan(i, i + 1, alpha=0.13, color=c, lw=0)

    ax_v.plot(t_ax, vels, color='#222', lw=0.9)
    if T > 20:
        win = min(30, T // 5)
        mv  = np.convolve(vels, np.ones(win) / win, mode='valid')
        ax_v.plot(range(win - 1, T), mv, color='gold', lw=2.0,
                  label=f'{win}-step avg')

    w_patch = plt.Rectangle((0,0),1,1, fc=SWIM_C  + '44', label='water')
    l_patch = plt.Rectangle((0,0),1,1, fc=CRAWL_C + '44', label='land')
    ax_v.legend(handles=[w_patch, l_patch], fontsize=8, loc='upper left')
    ax_v.set_xlim(0, T)
    ...
    # ── title & save ──────────────────────────────────────────────────────────
    title = (f'Body Profile & Gait Analysis — {phase_name}'
             if phase_name else 'Body Profile & Gait Analysis')
    fig.suptitle(title, fontsize=13, fontweight='bold')

    plt.savefig(save_path, dpi=130, bbox_inches='tight')
    plt.close(fig)
    print(f'🦠 Body profile figure saved: {save_path}')
