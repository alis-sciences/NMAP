#!/usr/bin/env python3
"""
progressive_ext_env.py
======================
Extended version of progressive_mixed_env.py.

Key difference: the reward function composes the dm_control swimmer base
velocity reward (forward_velocity × tolerance, producing episode returns in
the 200-1000 range matching the NCAP paper baseline) with the existing
medium-switching components (transition bonus, sustained swim bonus, mismatch
penalty, velocity rebalancing, Bayesian belief reward).

This makes episode returns directly comparable to the NCAP paper learning
curves while preserving all gait-switching reward structure.

Class names use the *Ext* suffix so both environments can coexist:
    ProgressiveSwimCrawlExt           -- dm_control task
    ProgressiveMixedSwimmerEnvExt     -- gym-style wrapper
    TonicProgressiveMixedWrapperExt   -- Tonic-compatible wrapper
"""
...
class ProgressiveSwimCrawlExt(swimmer.Swimmer):
    ...
    def __init__(self, 
        ...
        # 'on' is a user-friendly alias for 'proxy'
        if self._anisotropic_drag_mode == 'on':
            self._anisotropic_drag_mode = 'proxy'
    ...
    def get_reward(self, physics):
        """
        Composite reward = dm_control base velocity reward
                         + medium-switching reward components.

        The base velocity reward calls swimmer.Swimmer.get_reward(), which
        returns a tolerance-smoothed value in [0, 1] per step based on forward
        velocity relative to _desired_speed.  Summed over a 1000-step episode
        this produces returns in the 200-1000 range, matching the NCAP paper
        scale and making learning curves directly comparable.

        The switching components (_compute_switching_reward) preserve all
        existing gait-switching incentives: transition bonus with cooldown,
        sustained swim bonus, medium-mismatch penalty, velocity rebalancing,
        and Bayesian belief reward.  These appear as additive corrections on
        top of the base locomotion signal.
        """
        # ── 1. dm_control base velocity reward ───────────────────────────────
        # Returns [0, 1] per step; ~500 per episode for a good swimmer.
        # Lower when on land (slower velocity) — correct for gait comparison.
        base_velocity_reward = super().get_reward(physics)

        # ── 2. Medium-switching components ───────────────────────────────────
        # All existing incentives for gait switching, unchanged in magnitude.
        # Active from Phase 2 onward (training_progress >= 0.3).
        switching_reward = self._compute_switching_reward(physics)

        return base_velocity_reward + switching_reward

    def _compute_switching_reward(self, physics):
        """
        All medium-switching reward components extracted from the original
        get_reward().  Called by the compositor above; can also be called
        independently for diagnostics.

        Returns the sum of:
          • base_reward * 0.2   (activity/efficiency on land/water)
          • navigation_reward   (progress toward targets)
          • environment_transition_bonus (with cooldown)
          • sustained_swim_bonus
          • medium_mismatch_penalty
          • velocity_reward     (rebalanced for Phase 3+)
          • bayesian_reward
        ...
        joint_velocities = physics.data.qvel
        joint_activity = np.sum(np.abs(joint_velocities))
        baseline_activity_reward = min(joint_activity * 0.01, 0.1)
        ...
        if self._training_progress >= 0.3:
            try:
                ...
                if self._bayes_prev_entropy is not None:
                    delta_H = self._bayes_prev_entropy - H_norm
                    if delta_H > 0.0:
        ...
        if self._current_targets and self._current_target_index < len(self._current_targets):
            ...
            if hasattr(self, '_initial_target_distance') and self._initial_target_distance > 0:
                ...
                progress_reward = progress_ratio * 2.0 * time_factor * target_type_multiplier
                navigation_reward += progress_reward
                
                if self._target_visit_timer > 30:
                    ...
                    if recent_progress > 0.01:
                        navigation_reward += 0.2 * target_type_multiplier
            ...
            if self._target_visit_timer > 600:
                navigation_reward -= 0.02  
            if self._target_visit_timer > 1200:
                navigation_reward -= 0.05  
            if self._target_visit_timer > 1800:
                navigation_reward -= 0.1   
            
            if target_reached:
                target_completion_reward = 10.0 * target_type_multiplier
                navigation_reward += target_completion_reward
                
                try:
                    from tqdm import tqdm
                    time_taken = self._target_visit_timer / 30.0
                    ...
                    target_env = "🏝️ LAND" if current_target['type'] == 'land' else "🌊 WATER"
                    reward_info = f"(+{target_completion_reward:.1f} reward)"
                ...
                if self._current_target_index >= len(self._current_targets):
                    if self._targets_reached % 100 == 0:
                        try:
                            from tqdm import tqdm
...
try:
    @swimmer.SUITE.add()
    def progressive_swim_crawl_ext(
            n_links=6,
            desired_speed=_SWIM_SPEED,
        ...
        """Progressive swim and crawl task — ext version with dm_control base reward."""
        model_string, assets = swimmer.get_model_and_assets(n_links)
        physics = swimmer.Physics.from_xml_string(model_string, assets=assets)

        task = ProgressiveSwimCrawlExt(
    ...
    # progressive_swim_crawl_ext already registered — safe to ignore on re-import.
    pass

class ProgressiveMixedSwimmerEnvExt:
...
class TonicProgressiveMixedWrapperExt(gym.Env):
    """
    ...
    def __init__(self, n_links=5, time_feature=True, desired_speed=_SWIM_SPEED,
        ...
        self.env = ProgressiveMixedSwimmerEnvExt(
            n_links=n_links,
            desired_speed=desired_speed,
            expose_environment_observation=expose_environment_observation,
            expose_viscosity_observation=expose_viscosity_observation,
